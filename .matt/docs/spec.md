...
Fork-relevant entries:

- `messages.statusReactions.stickyError: true` and `messages.statusReactions.awaitTerminalCleanup: true` — opt into the fork's Discord reaction behaviour (see Patches).
- `models.anthropic.models[]` overlay pins `claude-opus-5` at `contextWindow: 1000000` — opus-5 is absent from the anthropic plugin catalog in v2026.7.1-2, so without the overlay it falls to the 128k `AGENT_CUSTOM_MODEL_DEFAULT_CONTEXT_WINDOW` and sessions overflow. `claude-fable-5`, `claude-sonnet-5`, `claude-mythos-5` are in the catalog at 1M — no pin needed. Note `agents.defaults.models` is a selection allowlist (`AgentModelRuntimeEntrySchema` is `.strict()`), NOT a catalog override — `contextWindow` there is rejected.
